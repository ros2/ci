...
class WindowsBatchJob(BatchJob):
    ...
    def _setup_compiler_cache(self):
        """
        Run every compilation through sccache.

        Returns without doing anything if sccache is not installed, rather
        than failing the job: the build is correct either way, only slower.
        That guard lives here and not in pre() so that it cannot also skip
        the MAX_PATH mitigation, which the Ninja generator needs whether or
