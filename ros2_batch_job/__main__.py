...
def process_coverage(args, job, packages_for_coverage_str=None):
    ...
    print(os.path.isfile(total_coverage_file))
    print(str(os.path.isfile(total_coverage_file)))

    # assert os.path.isfile(total_coverage_file), \
    #    'coverage file generated by lcov-result was not found at %s' % total_coverage_file
    ...
    cmd = ['lcov_cobertura', total_coverage_file, '--output', outfile]
    print(cmd)
    subprocess.run(cmd, check=True)
